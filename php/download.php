<?php
	...
	$settings = json_decode(file_get_contents(".conf.json"), true);
	$ip = $settings['ip'];
	...
	if( isset( $_GET['url'] ) && isset( $_GET['type'] ) )
	{
		...
		elseif( $_GET['type'] === 'get' )
		{
			...
			if( isset($_GET['resume']) && $_GET['resume'] == 'false' )
			{
				...
				$storage_loc = $result['storage_path'];
				if($ip == "" || $sharefoldername == "" || $username == "" || $password == "" || $location_id == ""){
					$storage_loc = "Downloads";
				}
				else{
					$storage_loc = $ip.'/'.$sharefoldername.'/'.$d.'/'.$filename;
				}

				$query = "INSERT INTO files VALUES( NULL, \"$filename\", \"$url\", 0, \"$type\", \"$ext\", \"n\",\"$support\", 0, \"$location_id\", \"$storage_loc\", \"$date\", \"$date\" )";
	...
	{
		global $type, $ip, $sharefoldername, $username, $password, $location_id;
		...
		if($ip == "" || $sharefoldername == "" || $username == "" || $password == "" || $location_id == ""){
			...
			$newFile = date('d-m-Y')."\\".$filename;
			shell_exec("smbclient //".$ip."/".$sharefoldername." -U ".$username."%".$password." -c \"mkdir ".date('d-m-Y')."\"");
			if(!(shell_exec("smbclient //".$ip."/".$sharefoldername." -U ".$username."%".$password." -c \"put ".$tmpFile." ".$newFile."\" & echo $!") && unlink($tmpFile ))){
				echo "event: mainerror\n";
				echo "data: Cannot Move Downloaded File To Destination Folder\n\n";
				ob_flush();
				flush();
