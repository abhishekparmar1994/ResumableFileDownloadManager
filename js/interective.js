...
stopMenu.addEventListener('click',function(event)
	{
		if(!this.classList.contains('deactive'))
		{
			...
			document.title = "NK Download Manager";
		}
	},false);
...
var manualsync = document.querySelector('.menuOptions span.manualsync');
manualsync.addEventListener('click',Checknewfile, false);
newFileCheckInterval = "";
function Checknewfile(){
	var request = new XMLHttpRequest;
	request.open('GET','php/check_new_file.php');
	request.onreadystatechange = function(){
		if( request.readyState === 4 && request.status == 200 ){
			...
			if(res.msg == "resume"){
				...
				newFileCheckInterval = "";
				var tempArr = document.querySelector('.allFiles tbody').children
				for(d = 0; d < tempArr.length; d++){
					if(tempArr[d].getAttribute('data-id') == res.data){
						tempArr[d].children[0].click();
				...
				clearInterval(newFileCheckInterval);
				newFileCheckInterval = "";
				document.querySelector('input[name="newUrl"]').value = res.data;
				document.querySelector('input[name="addurl"]').click();
			}else{
				if(newFileCheckInterval == ""){
					newFileCheckInterval = setInterval(function(){
						console.log("newFileCheckInterval");
						Checknewfile();
					}, 30000);
				}
			}
...
function stopChangingTitle( title )
{
	var timer = getTimer();
	clearInterval( timer );
	document.title = title || "NK Download Manager";
}
